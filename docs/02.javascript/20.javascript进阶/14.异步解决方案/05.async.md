---
title: async/await 异步解决方案
date: 2020-02-27 18:37:01
permalink: /pages/aaee3ssdse
author: 
  name: jeffery
categories: 
  - javascript
...
`async/await` 函数是` Generator` 函数的语法糖

使用 `Generator` 函数，依次读取两个文件代码如下
...
由于`async`函数返回的是 `Promise` 对象，可以作为`await`命令的参数。所以，上面例子也可写成下面形式
...
可以看到 `Async` 函数的实现最简洁，最符合语义，几乎没有语义不相关的代码。它将Generator写法中的自动执行器，改在语言层面提供，不暴露给用户，因此代码量最少。如果使用 `Generator` 写法，自动执行器需要用户自己提供
