...
```ts
let sym: symbol = Symbol('jeffery')
sym = Symbol('jeffery') // OK
sym = 'jeffery' // Error
```
...
// 任意类型
const anyArray: Array<any> = ['jeffery', 18, {}]
```
...
```ts
const tuple: [string, number] = ['jeffery', 18]
...
```ts
type Name = 'jeffery' | 'jeffery1996' | 'jeffery'
```
