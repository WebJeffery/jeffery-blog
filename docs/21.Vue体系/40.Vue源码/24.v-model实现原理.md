...
`AST`产生阶段对事件指令`v-on`的处理是为`AST`树添加`events`属性。类似的，普通指令会在`AST`树上添加`directives`属性，具体看`addDirective`函数。
...
![img](https://gitee.com/FIF/pic-beg/raw/master/images/javascript/11.2.png)
